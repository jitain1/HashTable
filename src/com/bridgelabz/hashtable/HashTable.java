...
public class HashTable<K, V> {
    ...
    int getIndex(K key) {
        int index = Math.abs(key.hashCode()) % bucketSize; // absolute value -> (-ve) convert (+ve value)
        return index;
    }
...
//            return myNode != null ? myNode.value : null;
    ...
    public void add(K word, V frequency) {
        ...
        if (myLinkedList == null) {
            ...
            myBucketArray.add(index, myLinkedList); // ArrayList - add method
            myLinkedList.append(word, frequency);  // LinkedList - append method
        } else {
            MyNode<K, V> myNode = myLinkedList.search(word);
            if (myNode == null) {
